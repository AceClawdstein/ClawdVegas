...
import { evaluateHand, type HandResult } from './hand-eval.js';
import { calculatePots, awardPots, consolidateAwards, type PlayerContribution } from './pot.js';
import { type Card, cardDisplay } from './deck.js';
...
export class PokerTable extends EventEmitter {
  ...
  private handStartTimeout: NodeJS.Timeout | null = null;
  ...
  getStateForPlayer(address: string): GameState {
    const seats = this.state.seats.map(seat => {
      if (!seat) return null;
      if (seat.address === address) return seat;
      return { ...seat, holeCards: null };
    });
    return { ...this.state, seats, deck: [] };
  }
  ...
  sit(address: string, seatIndex: number, buyIn: bigint): TableResult<{ seat: Player }> {
    // Validate seat
  ...
  stand(address: string): TableResult<{ chips: bigint }> {
  ...
  addChips(address: string, amount: bigint): TableResult<{ newStack: bigint }> {
    const seatIndex = this.getSeatByAddress(address);
    if (seatIndex < 0) {
      return { success: false, error: 'Not seated at this table' };
    }
  ...
  act(address: string, action: PlayerAction): TableResult<{ action: PlayerAction }> {
  ...
  private advanceGame(): void {
    ...
    const bettingPlayers: BettingPlayer[] = this.state.seats
      .filter((s): s is Player => s !== null && s.holeCards !== null)
      .map(p => ({
        address: p.address,
        stack: p.stack,
  ...
  private awardPotToLastPlayer(winner: Player): void {
    // Calculate total pot
    const contributions: PlayerContribution[] = this.state.seats
      .filter((s): s is Player => s !== null && s.holeCards !== null)
      .map(p => ({
        address: p.address,
  ...
  private goToShowdown(): void {
    ...
    // Evaluate all hands
    const handResults: Array<{ player: Player; hand: HandResult }> = [];
    const showdownHands: Array<{ seatIndex: number; cards: Card[]; handName: string }> = [];
    ...
    for (const player of playersInHand) {
      ...
      showdownHands.push({
        ...
        cards: player.holeCards as Card[],
    ...
    const foldedPlayers = new Set(
      this.state.seats
        .filter((s): s is Player => s !== null && s.isFolded)
        .map(p => p.address)
    );
    ...
    const newSeats = [...this.state.seats];
    const winnerInfo: Array<{ seatIndex: number; amount: string }> = [];
  ...
  private completeHand(newSeats: (Player | null)[]): void {
  ...
  getValidActionsFor(address: string): ValidActions | null {
    ...
    if (seatIndex !== this.state.activePosition) return null;
 ...
 */
export function createTable(config?: Partial<TableConfig>): PokerTable {
