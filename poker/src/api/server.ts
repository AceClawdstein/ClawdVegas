...
async function runDemoHand(agents: DemoAgent[], log: string[], actionDelayMs: number = 100): Promise<void> {
...
let playActionResolve: ((action: { action: string; amount?: string }) => void) | null = null;
 ...
 */
app.post('/api/play/start', async (req: Request, res: Response) => {
 ...
 */
app.post('/api/play/action', (req: Request, res: Response) => {
  const { address, action, amount } = req.body as { address?: string; action?: string; amount?: string };
 ...
 */
async function runPlaySession(): Promise<void> {
  ...
  while (playSession?.isActive) {
    ...
    while (!activePhasesSet.has(table.getState().phase)) {
      await new Promise(r => setTimeout(r, 200));
      if (!playSession?.isActive) return;
    }
 ...
 */
app.post('/api/play/stop', (_req: Request, res: Response) => {
  if (playSession) {
