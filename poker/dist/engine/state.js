 ...
 */
import { createDeck, shuffleDeck, dealHoleCards, dealCards } from './deck.js';
import { createBettingStateWithBlinds, resetForNewRound } from './betting.js';
...
export function getActivePlayers(state) {
    return state.seats.filter((s) => s !== null && !s.isFolded && !s.isSittingOut);
}
/**
 * Get players still in the hand (not folded)
 */
export function getPlayersInHand(state) {
    return state.seats.filter((s) => s !== null && !s.isFolded && s.holeCards !== null);
}
/**
 * Get players who can still act (not folded, not all-in)
 */
export function getPlayersWhoCanAct(state) {
    return getPlayersInHand(state).filter(p => !p.isAllIn);
}
/**
 * Find next position to act (clockwise from current)
 */
export function findNextActivePosition(state, fromPosition) {
 ...
 */
export function getFirstToActPreflop(state) {
 ...
 */
export function getFirstToActPostflop(state) {
    return findNextActivePosition(state, state.buttonPosition);
}
/**
...
export function canStartHand(state) {
    const activePlayers = state.seats.filter((s) => s !== null && !s.isSittingOut && s.stack > 0n);
 ...
 */
export function dealCommunityCards(state) {
    const events = [];
    let newPhase;
    let newCommunity;
    let newDeck;
 ...
 */
export function shouldGoToShowdown(state) {
 ...
 */
export function allPlayersAllIn(state) {
