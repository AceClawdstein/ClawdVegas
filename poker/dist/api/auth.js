 ...
 */
export async function verifyChallenge(wallet, signature, nonce, message) {
    const normalized = wallet.toLowerCase();
    ...
    const token = jwt.sign({ wallet: normalized, iat: Math.floor(Date.now() / 1000) }, JWT_SECRET, { expiresIn: JWT_EXPIRY });
