...
import { evaluateHand } from '../engine/hand-eval.js';
...
function getHandStrength(holeCards, communityCards) {
    if (communityCards.length === 0) {
        ...
        if (!c1 || !c2)
            return { strength: 20, handName: 'Unknown', description: 'No cards' };
 ...
 * Calculate pot odds
 */
function getPotOdds(callAmount, pot) {
    if (callAmount === 0n)
        return 100;
 ...
 */
function buildDecisionPrompt(context, personality) {
    const handStrength = getHandStrength(context.myCards, context.communityCards);
    const potOdds = getPotOdds(BigInt(context.validActions.callAmount), BigInt(context.pot));
    // Format hand history
    const historyStr = context.handHistory.actions.length > 0
        ? context.handHistory.actions.map(a => `${a.player} ${a.action}${a.amount ? ' $' + a.amount : ''} (${a.phase})`).join('\n')
    ...
    const actionsStr = [];
    if (context.validActions.canFold)
        actionsStr.push('FOLD');
    if (context.validActions.canCheck)
        actionsStr.push('CHECK');
    if (context.validActions.canCall)
        actionsStr.push(`CALL $${context.validActions.callAmount}`);
    if (context.validActions.canBet)
        actionsStr.push(`BET ($${context.validActions.minBet} - $${context.validActions.maxBet})`);
    if (context.validActions.canRaise)
        actionsStr.push(`RAISE ($${context.validActions.minRaise} - $${context.validActions.maxBet})`);
 ...
 */
export async function makeDecision(context, personality) {
...
export class LLMAgent {
     ...
     */
    recordAction(player, action, amount, phase) {
        const entry = { player, action, phase };
        if (amount !== undefined) {
            entry.amount = amount;
        }
     ...
     * Make a decision
     */
    async decide(myCards, myStack, myCurrentBet, communityCards, pot, phase, opponents, validActions, isButton, position) {
        const context = {
            myName: this.name,
            myCards,
            myStack,
